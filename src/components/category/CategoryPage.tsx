...
export function CategoryPage({ 
  ...
  // derive unique brands and categories from products
  const brands = Array.from(new Set(products.map(p => p.brand).filter(Boolean))) as string[]
  const cats = Array.from(new Set(products.map(p => p.category))) as string[]
  const [brandSel, setBrandSel] = useState(searchParams.brand || '')
  const [catSel, setCatSel] = useState(searchParams.cat || '')
  ...
  const updateUrl = (newParams: Record<string, string | null>) => {
    ...
    const base = gender === 'all' ? `/${category.slug}` : `/${gender}/${category.slug}`;
  ...
  const clearFilters = () => {
    ...
    setBrandSel('')
  setCatSel('')
  updateUrl({ min_price: null, max_price: null, brand: null, cat: null, page: null })
  }
                  ...
                  <div className="space-y-4">
                    <div className="flex space-x-4">
                      <div className="flex-1">
                        ...
                        <input
                          ...
                          className="w-full px-4 py-3 border-2 border-gray-300 focus:border-black focus:ring-0 outline-none transition-colors duration-300 bg-white text-black"
                          placeholder="0"
                        />
                      </div>
                      <div className="flex-1">
                    ...
                    <div className="flex space-x-3">
                      <button
                        ...
                        className="bg-black text-white px-6 py-3 font-semibold hover:bg-gold-500 hover:text-black transition-all duration-300"
                      >
                        Apply Filter
                      </button>
                      <button
                        onClick={clearFilters}
                        className="border-2 border-black text-black px-6 py-3 font-semibold hover:bg-black hover:text-white transition-all duration-300"
                      >
                        Clear All
                      </button>
                    </div>
